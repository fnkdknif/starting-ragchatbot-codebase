...
class TestAIGeneratorToolCalling:
    ...
    def test_generate_response_without_tools(
        self, test_config, mock_anthropic_response_no_tools
    ):
        """Test basic response generation without tools"""
        ai_gen = AIGenerator(test_config.ANTHROPIC_API_KEY, test_config.ANTHROPIC_MODEL)

        with patch.object(
            ai_gen.client.messages,
            "create",
            return_value=mock_anthropic_response_no_tools,
        ):
    ...
    def test_tool_definitions_passed_correctly(self, test_config, tool_manager):
        ...
        with patch.object(
            ai_gen.client.messages, "create", return_value=mock_response
        ) as mock_create:
    ...
    def test_tool_execution_triggered(
        self, test_config, tool_manager, vector_store_with_data
    ):
